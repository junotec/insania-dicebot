...
def simple_dice(dice_num, dice_size)
    ...
    msg = 'Insania: '+ dice_num.to_s + 'd' + dice_size.to_s + ' > ' + dice_val.to_s + ' > ' + dice_val.sum.to_s
    return msg
end
...
def judge_dice(dice_num, dice_size, condition, value)
    ...
    msg = 'Insania: '+ dice_num.to_s + 'd' + dice_size.to_s + condition + value.to_s + ' > ' + dice_val.to_s + ' > ' + dice_val.sum.to_s + ' > ' + result
    return msg
end
...
def judge_dice_with_formula(dice_num, dice_size, condition, value, formula)
    ...
    msg = 'Insania: '+ dice_num.to_s + 'd' + dice_size.to_s + condition + formula_for_markdown + ' > ' + dice_num.to_s + 'd' + dice_size.to_s + condition + value.to_s + ' > ' + dice_val.to_s + ' > ' + dice_val.sum.to_s + ' > ' + result
    return msg
end
...
def res(active, passive)
    ...
    msg = 'Insania: 1d100<=' + value.to_s + ' > ' + dice_value.to_s + ' > ' + result
    return msg
end
...
def temp_madness()
    ...
    roll = f.read.split(/\R/)
    f.close
    msg = roll[dice(10)] + " ―  一時的狂気(" + (dice(10)+4).to_s + "ラウンドまたは" + (dice(6)*10+30).to_s + '分)'
    return msg
end
