...
bot.message do |event|
    ...
    # 正規表現で扱いやすいように主要な全角文字を半角に、大文字Dを小文字に、不等号イコールの表記を揃える
    content = content_raw.tr('０-９ａ-ｚＡ-Ｚ＜＝＞＋－＊','0-9a-zA-Z<=>+\-*').gsub(/D|=<|=>|　/, "D" => "d", "=<" => "<=", "=>" => ">=", "　" => "\s")
    ...
    # <=ダイス
    elsif /^\d+d\d+<=\d+$/ =~ content || /^\d+d\d+<=\d+\s.*$/ =~ content then
    ...
    # 複数ダイス。ダイスを各自振った後元の式に代入し、evalでそのまま計算。
        ...
        for i in 0..content_dices.length - 1 do
            ...
            content_converted = content_converted.sub(dice_nums[i]+"d"+dice_sizes[i], results[i].to_s)
        end 
        k = eval(content_converted)
        event.respond '\> ' + get_user_name(event.user) + "\n"  + 'Insania: '+ content_nbsp.match(/.*?(?=\s)/)[0].gsub('*', '\*') + ' > ' + content_converted.gsub('*', '\*') + ' > ' + k.to_s
    end
    
    # 従来の対抗ロール。
    if /^res\(\d+-\d+\)\s.*$/ =~ content then
        md = content.match(/(?<=\().*?(?=-)/)
        nd = content.match(/(?<=-).*?(?=\))/)
        active = md[0]
        passive = nd[0]
...
bot.command :choice do |event, *options|
    event.respond '\> ' + get_user_name(event.user) + "\nInsania: choice" + options.to_s + " > " + options[rand(options.length)]
end
...
bot.command :statusgen do |event, name|
    begin
        if sheet_locked?(name) == true then
            ...
            event.respond "そのような名前のシートは存在しません。(" + "[" + e1.class.to_s + "] " + e1.message.to_s + ")"
            print_error(e1)
        rescue =>e2
...
bot.command :change do |event, name, skill, value, type|
    begin
        if sheet_locked?(name) == true then
            event.respond "シート「"+ name +"」はロックされています。"
        elsif type_abbr_to_type(type) == "合計値" then
            event.respond "合計値は手動変更できません。"
        else
            change_value(name, skill, value, type)
            event.respond "「" + name + "」の" + skill + "に"+ type_abbr_to_type(type) + value + "を追加しました。"
        end
    rescue =>e
        event.respond "[" + e.class.to_s + "] " +  e.message.to_s
        print_error(e)
...
bot.command :show do |event, name, skill, type="合計値"|
    begin
        event.respond skill + "(" + type_abbr_to_type(type) + "): " + show_value(name, skill, type)
    rescue NoMethodError => e1
        event.respond "そのような名前のシートは存在しません。(" + "[" + e1.class.to_s + "] " + e1.message.to_s + ")"
        print_error(e1)
    rescue =>e2
...
bot.command :showsheet do |event, name|
    begin
      ...
      if msg.length > 2000 then
        ...
        for i in 0...msgs.length do
          event.respond msgs[i]
        end
      else
        event.respond msg
      end
...
bot.command :delsheet do |event, name|
    begin
      if name == "対応表" || name == "Sheet1" then
          event.respond "そのシートは削除できません。"
      else
          event.user.await(:confirm) do |confirm_event|
              if confirm_event.message.content == name then
                delete_sheet(name)
...
# シートを利用した自動判定。簡単な計算機能付き。オプションで判定の種類とダイスを変更可
bot.command :i do |event, id, skill, dice_num = "1", dice_size = "100", type = "<="|
    begin
        if skill =~ /\A[^*\+\-\/]+[*\/\+\-]\d+[\d\+\-*\/\(\)]*\z/ then
            skill_name = skill.gsub(/\A([^*\+\-\/]+)[*\/\+\-]\d+[\d\+\-*\/\(\)]*\z/, '\1')
            skill_val = skill_value(id_to_title(id.to_i), skill_name)
            formula = skill.sub(/\A[^*\+\-\/]+([*\/\+\-]\d+[\d\+\-*\/()]*)\z/, skill_val.to_s + '\1')
            value = eval(formula)
            event.respond '\> ' + get_user_name(event.user) + "\n" + judge_dice_with_formula(dice_num.to_i, dice_size.to_i, type, value, formula)
        else
            skill_val = skill_value(id_to_title(id.to_i), skill)
            event.respond '\> ' + get_user_name(event.user) + "\n" + judge_dice(dice_num.to_i, dice_size.to_i, type, skill_val)
        end
    rescue NoMethodError => e1
        event.respond "そのようなIDのシートは存在しません。(" + "[" + e1.class.to_s + "] " + e1.message.to_s + ")"
        print_error(e1)
    rescue ArgumentError => e2
        event.respond "技能名かIDが間違っています。(" + "[" + e2.class.to_s + "] " + e2.message.to_s + ")"
        print_error(e2)
    rescue =>e3
...
bot.command :san do |event, id, value|
    begin
        ...
        event.respond "SAN" + (show_value(id_to_title(id.to_i), "正気度", "sum").to_i + value.to_i).to_s + "→" + show_value(id_to_title(id.to_i), "正気度", "sum")
    rescue =>e
        event.respond "[" + e.class.to_s + "] " +  e.message.to_s
        print_error(e)
    end
