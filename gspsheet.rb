...
def status_gen(pc_title)
    ws = $sp.worksheet_by_title(pc_title)
    # worksheet.listは1行目の内容を列のタイトルとして、
    # 列番号の代わりにそのタイトルとセル内容を
    # 関連付けたハッシュ(1行1つ)の配列。
    # この場合の行指定は、2行目が「0」となる。
    # (通常は左上のセルが[1,1]となる)
    for i in 0..4 do # STR,CON,POW,DEX,APP
        ws.list[i]["初期値"] = dices(3,6).to_s
    end
    for i in 5..6 do # SIZ,INT
...
# 1行目の内容を列タイトルとして、そのタイトルから
# 列番号を引く。列数が膨大になるのは考えづらいので
# 工夫なく線形探索で済ませている
def column_title_to_column(ws, column_title)
  ...
  raise(ArgumentError, format('Found no column with the title provided: %p', column_title))
end
...
def reload_skills(pc_title)
  ...
  while ws[i,1] != "" do
    ...
    if ws.list[i-2]["合計値"] == "" then
      ...
      ws.list[i-2]["合計値"] = "=SUM("+ini_val_cell+":"+hob_val_cell+")-"+minus_val_cell
      ws.save if ws.dirty?
    end
    i += 1
  end
...
def add_value(pc_title, skill, value, type)
    ...
    # scanでハッシュに変換
    skills = JSON.parse(skills_json)
    ws.list[skills[skill]-2][type_abbr_to_type(type)] = (ws.list[skills[skill]-2][type_abbr_to_type(type)].to_i + value.to_i).to_s
    ws.save
end
...
def show_sheet(name)
    ...
    msg = ""
    for i in 1..ws.num_rows do #行
        for j in 1..ws.num_cols do #列
            msg = msg + " \| " + ws[i,j]
        end
        msg = msg + "\n"
    end
